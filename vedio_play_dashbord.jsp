<%-- 
    Document   : vedio_play_dashbord
    Created on : 27 Feb, 2022, 12:42:43 AM
    Author     : rohit
--%>
...
String channel_code="";
String channel_name="";
                            ResultSet rs3 = st.executeQuery("select * from vedio where code<>'" + vedio_code + "' AND status='" + 1 + "'ORDER BY RAND () ");
                            while (rs3.next()) {
                                String code = rs3.getString("code");
                                String title = rs3.getString("title");
                                String date = rs3.getString("dt");
                                ResultSet rs2 = st2.executeQuery("select * from vedio where code='" + code + "' AND status='1' ");

                                if (rs2.next()) {

                                    channel_code = rs2.getString("channel_code");
                                    ResultSet rs4 = st2.executeQuery("select * from channel where code='" + channel_code + "' ");

                                    if (rs4.next()) {
                                        channel_name = rs4.getString("channel_name");
                                    }

                                }
                                int views_count = 0;
                                ResultSet rs5 = st2.executeQuery("select count(*) from views where vedio_code='" + code + "'");
                                if (rs5.next()) {
                        ...
                        <div class="col-sm-3" style="margin-top:2%;">
                            <a href="vedio_play_secreen.jsp?code=<%=code%>&title= <%=title%>" style="color:black" class="views" id="<%=code%>">
                                <video   width="230" 
                                         height="150" 
                                         poster=
